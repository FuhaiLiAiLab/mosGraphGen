...
class NetAnalyse():
    ...
    def fold_avg(self, fold_n):
        # first
        fold_first_edge_weight_df = pd.read_csv('./ROSMAP-analysis/fold_' + str(fold_n) +'/first_edge_weight.csv')
        ...
        # block
        fold_block_edge_weight_df = pd.read_csv('./ROSMAP-analysis/fold_' + str(fold_n) +'/block_edge_weight.csv')
        fold_block_edge_weight_df = fold_block_edge_weight_df.drop(['EdgeType'], axis=1)
        # last
        fold_last_edge_weight_df = pd.read_csv('./ROSMAP-analysis/fold_' + str(fold_n) +'/last_edge_weight.csv')
        ...
        # [weight]
        fold_avg_edge_weight_df.to_csv('./ROSMAP-analysis/fold_' + str(fold_n) +'/fold_avg_edge_weight_df.csv', index=False, header=True)

    def all_avg(self, k):
        # all fold_avg_edge_weight_df
        fold_avg_edge_weight_df_list = []
        for fold_n in range(1, k + 1):
            fold_avg_edge_weight_df = pd.read_csv('./ROSMAP-analysis/fold_' + str(fold_n) +'/fold_avg_edge_weight_df.csv')
